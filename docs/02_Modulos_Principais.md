...
## 1. Artificer (Dalamud Plugin API)
...
## 2. Artificer.Simulator (Simulação Determinística)
...
## 3. Artificer.Solver (Inteligência & Busca)
Responsabilidades Técnicas:
- O cérebro reside no framework **MCTS** (Monte Carlo Tree Search), desenhado em C# com extensivo uso de `unsafe` blocks.
- **RaphaelUtils / Raphael.Net:** O Solver no C# interage com o `Raphael.Net` (implementado em Rust) para processar os nós de MCTS na simulação em velocidade nativa. Usa interoperabilidade direta com o C. O arquivo `RaphaelUtils.cs` mapeia Enums da API `Artificer.Simulator` para o lado nativo (`Raphael.Action`).
