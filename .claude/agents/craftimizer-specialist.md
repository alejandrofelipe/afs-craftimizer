...
```powershell
.\scripts\bump-version.ps1 -Type minor   # nova feature
.\scripts\bump-version.ps1 -Type patch   # bug fix
.\scripts\bump-version.ps1 -Type build   # refactor/chore
.\scripts\bump-version.ps1 -Type major   # breaking change
```

### 3. Build e Deploy
```powershell
dotnet build Craftimizer/Craftimizer.csproj -c Release
$destPath = "$env:APPDATA\XIVLauncher\installedPlugins\Craftimizer\$(([xml](Get-Content Craftimizer/Craftimizer.csproj)).Project.PropertyGroup[0].Version)"
...
- [ ] Build de release gerado e deployado localmente
- [ ] Teste in-game necessário (mencionar ao usuário)

**IMPORTANTE:** Sempre mencione ao usuário quando teste manual in-game for necessário.
